#!/usr/bin/env bash
# prod_reg_test_d_roll180_width45_capture.sh — TEST D: Production regression test (45mm, roll 180),
# 180-DEGREE ROLL DISCRIMINATOR (2026-08-22).
#
...
#   roll=pi -> 30mm (M8). Roll and width have therefore been perfectly
#   confounded in every run to date. On a CUBE, roll no longer selects the
#   width: this run is 45 mm AT roll=pi, isolating roll as a single
#   variable against M7's 45 mm at roll=0.
#     M7   45mm roll=0     theta 0.402893  stable, 96.4% ceiling, 0.104mm slip
#     M8   30mm roll=pi  theta 0.538015  ~80Hz limit cycle, 7.984mm slip
#     M8.5 45mm roll=pi  theta 0.402893  <- this run
...
#     5. output prefix prod_reg_test_d_roll180_width45_* so NOTHING overwrites the
...
LOGDIR="docs"
SIM_LOG="${LOGDIR}/prod_reg_test_d_roll180_width45_sim_${TS}.log"
MG_LOG="${LOGDIR}/prod_reg_test_d_roll180_width45_movegroup_${TS}.log"
GRASP_LOG="${LOGDIR}/prod_reg_test_d_roll180_width45_grasp_${TS}.log"
STALL_CSV="${LOGDIR}/prod_reg_test_d_roll180_width45_stalls_${TS}.csv"
STALL_LOG="${LOGDIR}/prod_reg_test_d_roll180_width45_stallmonitor_${TS}.log"
TRAJ_CSV="runs/prod_reg_test_d_roll180_width45_traj_${TS}.csv"
TRAJ_LOG="${LOGDIR}/prod_reg_test_d_roll180_width45_trajrecorder_${TS}.log"
CONTACT_L="${LOGDIR}/prod_reg_test_d_roll180_width45_contact_left_${TS}.log"
CONTACT_R="${LOGDIR}/prod_reg_test_d_roll180_width45_contact_right_${TS}.log"
POSE_CSV="runs/prod_reg_test_d_roll180_width45_pose_${TS}.csv"
POSE_LOG="${LOGDIR}/prod_reg_test_d_roll180_width45_poserecorder_${TS}.log"
MARKER_DIR="${LOGDIR}/prod_reg_test_d_roll180_width45_markers_${TS}"
CSV_PATH="m3_grasp_trace_prod_reg_test_d_roll180_width45_${TS}.csv"
...
ok = (list(scene["object"]["size"]) == [0.045, 0.045, 0.045]
      ...
      and abs(tg - 0.402893) < 5e-7 and abs(abs(co[1]) - 1.0) < 1e-9
...
# ---------------------------------------------------------------------------
sec "2.5 IK REACHABILITY GATE — pick side AND place side, at roll=pi"
# tool0 orientation at roll=pi is a 180-deg rotation about X -> q=(1,0,0,0),
...
ik_check() {  # $1=x $2=y $3=z $4=label
  ...
  OUT=$(ros2 service call /compute_ik moveit_msgs/srv/GetPositionIK \
    "{ik_request: {group_name: 'arm', ik_link_name: 'tool0', robot_state: {is_diff: true}, avoid_collisions: true, pose_stamped: {header: {frame_id: 'world'}, pose: {position: {x: $1, y: $2, z: $3}, orientation: {x: 0.70710678, y: -0.70710678, z: 0.0, w: 0.0}}}, timeout: {sec: 5, nanosec: 0}}}" 2>&1)
