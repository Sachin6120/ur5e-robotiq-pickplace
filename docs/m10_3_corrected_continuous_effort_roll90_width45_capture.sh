#!/usr/bin/env bash
# m10_3_corrected_continuous_effort_roll90_width45_capture.sh — M10.3-CORRECTED: ONE full pick-and-place cycle (corrected continuous direct effort),
# M10.3-CORRECTED CONTINUOUS DIRECT EFFORT HOLD VALIDATION (2026-08-22).
#
# THE ONE CHANGE UNDER TEST
#   config/scene.yaml object.size: [0.030,0.045,0.045] -> [0.045,0.045,0.045]
#   With a NON-cube, gripper_roll selects the width: roll=0 -> 45mm (M7),
#   roll=pi/2 -> 30mm (M8). Roll and width have therefore been perfectly
#   confounded in every run to date. On a CUBE, roll no longer selects the
#   width: this run is 45 mm AT roll=pi/2, isolating roll as a single
#   variable against M7's 45 mm at roll=0.
#     M7   45mm roll=0     theta 0.402893  stable, 96.4% ceiling, 0.104mm slip
#     M8   30mm roll=pi/2  theta 0.538015  ~80Hz limit cycle, 7.984mm slip
#     M8.5 45mm roll=pi/2  theta 0.402893  <- this run
#   If the M8 limit cycle DISAPPEARS here, the trigger follows width/theta.
#   If it PERSISTS, roll/orientation remains a causal candidate.
#   NOTHING else changes: tcp_offset 0.120405, effort limit 1.0 N*m,
#   friction, contact params, controller config, all source files.
#
# WHAT THIS VALIDATES
...
#     5. output prefix m10_3_corrected_continuous_effort_roll90_width45_* so NOTHING overwrites the
...
LOGDIR="docs"
SIM_LOG="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_sim_${TS}.log"
MG_LOG="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_movegroup_${TS}.log"
GRASP_LOG="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_grasp_${TS}.log"
STALL_CSV="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_stalls_${TS}.csv"
STALL_LOG="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_stallmonitor_${TS}.log"
TRAJ_CSV="runs/m10_3_corrected_continuous_effort_roll90_width45_traj_${TS}.csv"
TRAJ_LOG="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_trajrecorder_${TS}.log"
CONTACT_L="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_contact_left_${TS}.log"
CONTACT_R="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_contact_right_${TS}.log"
POSE_CSV="runs/m10_3_corrected_continuous_effort_roll90_width45_pose_${TS}.csv"
POSE_LOG="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_poserecorder_${TS}.log"
MARKER_DIR="${LOGDIR}/m10_3_corrected_continuous_effort_roll90_width45_markers_${TS}"
CSV_PATH="m3_grasp_trace_m10_3_corrected_continuous_effort_roll90_width45_${TS}.csv"
...
# ---------------------------------------------------------------------------
sec "0.5 Confirm the DERIVED closing axis (must be object X) and width (0.045)"
...
ok = (list(scene["object"]["size"]) == [0.045, 0.045, 0.045]
      and r["axis_index"] == 0 and abs(r["width_m"] - 0.045) < 1e-12
      and abs(tg - 0.402893) < 5e-7 and abs(abs(co[0]) - 1.0) < 1e-9
      and abs(float(scene["grasp"]["tcp_offset"]) - 0.120405) < 1e-12)
