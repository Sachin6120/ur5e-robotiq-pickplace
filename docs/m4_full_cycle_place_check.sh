#!/usr/bin/env bash
# m4_full_cycle_place_check.sh — one-off variant of
# m3_run_full_cycle_trial_live.sh for M4's deliverable ("full loop incl.
# place and retreat -- one annotated run log"), NOT wired into
# scripts/11_m3_cycles.sh and not intended to replace it.
#
# WHY A SEPARATE SCRIPT RATHER THAN EDITING THE VALIDATED TRIAL SCRIPT
#   The M3 sweep's own CSV never samples the object after TRANSPORT_DONE --
#   watch_markers() in 11_m3_cycles.sh breaks out on .transportdone_ready or
#   .run_summary_ready, whichever comes first, and no marker file exists for
#   RETREAT_DONE at all. So every "SUCCESS" row in every M3 CSV to date is
#   evidence that the release/retreat STAGES EXECUTED, not evidence the
#   object ended up at object.place_pose -- the same class of gap this
#   project has already named twice (attachObject is not evidence of a
#   grasp; an achieved joint angle is not evidence of a grasp). Editing the
#   shared trial script to add a placement sample would change what every
#   future sweep cycle costs and risk perturbing a harness that took
#   multiple sessions to get reliable, for a check M4 only needs once.
#
# THE ONE ADDITION, on top of an otherwise-identical copy of
# m3_run_full_cycle_trial_live.sh: in section 5's poll loop, the moment
# "RETREAT_DONE" appears in the streamed log (arm has retreated, object has
# been sitting released and untouched since Stage 6), take ONE settled
# Gazebo-ground-truth sample of pick_target via sample_pose.py, BEFORE
# Gate-AFTER or teardown touch anything. No pass/fail threshold is applied
# here -- the spec gives M4 an annotated log, not a numeric gate; the
# comparison against object.place_pose is written up by hand afterward.
#
# Everything below this point is the original script, unchanged except for
# that one insertion (search PLACEMENT_SAMPLE).
#
...
DEADLINE=$(( $(date +%s) + 100 ))
RC=1
PLACEMENT_LOG="${LOGDIR}/m4_placement_${TS}.txt"
PLACEMENT_SAMPLED=0
while true; do
  # PLACEMENT_SAMPLE: the one addition over m3_run_full_cycle_trial_live.sh.
  # Fires once, the first time RETREAT_DONE shows up in the streamed log --
  # arm has cleared the object and nothing has touched it since Stage 6
  # (release). Sim is still fully up at this point (RUN SUMMARY, which
  # triggers teardown below, has not been seen yet). Runs synchronously
  # (blocks this poll loop for its own duration, up to --timeout-s) --
  # deliberate: RUN SUMMARY is expected to already be true by the time this
  # returns, so no cycle time is lost, and it means the sample is taken
  # before Gate-AFTER or teardown can touch anything.
  if [[ "$PLACEMENT_SAMPLED" -eq 0 ]] && grep -q "RETREAT_DONE" "$GRASP_LOG" 2>/dev/null; then
    printf '  RETREAT_DONE seen -- sampling pick_target ground-truth pose now, before teardown\n'
    python3 scripts/lib/sample_pose.py --topic /world/empty/pose/info \
      --entities pick_target --window-s 1.0 --min-samples 10 --tol-m 0.0005 \
      --timeout-s 15 > "$PLACEMENT_LOG" 2>&1
    PLACEMENT_RC=$?
    printf '  placement sample rc=%s -> %s\n' "$PLACEMENT_RC" "$PLACEMENT_LOG"
    cat "$PLACEMENT_LOG"
    PLACEMENT_SAMPLED=1
  fi
...
printf '\ndone: %s\n' "$(date -Is)"
printf 'artifacts:\n  sim_log=%s\n  movegroup_log=%s\n  grasp_log=%s\n  stall_csv=%s\n  grasp_csv=%s\n  placement_log=%s\n' \
  "$SIM_LOG" "$MG_LOG" "$GRASP_LOG" "$STALL_CSV" "$CSV_PATH" "$PLACEMENT_LOG"
