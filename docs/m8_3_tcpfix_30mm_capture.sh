#!/usr/bin/env bash
# m8_3_tcpfix_30mm_capture.sh — M8.3: ONE controlled FULL pick-and-place cycle
# testing the SINGLE-PARAMETER tcp_offset correction (2026-08-22).
#
# THE ONE CHANGE UNDER TEST
#   config/scene.yaml grasp.tcp_offset: 0.120405 -> 0.122233 m
#   0.120405 was a scalar calibrated at the 45 mm aperture (theta ~ 0.4021);
#   gripper_geometry.tcp_offset_m() reproduces it there to 13 microns. At the
#   30 mm aperture (theta = 0.5380147628) the same function gives 0.122233 m,
#   so the fixed scalar drove the descent 1.814 mm too deep, seating the pad
#   patch centre 2.266 mm below the object centre with only 1.22 mm of
#   bottom-edge clearance (M7, which lifted cleanly, sat at -0.766 mm with
#   3.04 mm clearance). This run tests whether correcting that seating
#   removes the ~8 mm lift-off drop. NOTHING ELSE IS CHANGED: not the
#   1.0 N*m effort limit, not friction, not physics, not geometry, not gains.
#
# WHAT THIS VALIDATES
...
#     5. output prefix m8_3_tcpfix_30mm_* so NOTHING overwrites the
#        m6_* / m7_fullcycle_axisfix_* / m8_30mm_squeeze_* /
#        m8_fullcycle_30mm_* evidence.
...
LOGDIR="docs"
SIM_LOG="${LOGDIR}/m8_3_tcpfix_30mm_sim_${TS}.log"
MG_LOG="${LOGDIR}/m8_3_tcpfix_30mm_movegroup_${TS}.log"
GRASP_LOG="${LOGDIR}/m8_3_tcpfix_30mm_grasp_${TS}.log"
STALL_CSV="${LOGDIR}/m8_3_tcpfix_30mm_stalls_${TS}.csv"
STALL_LOG="${LOGDIR}/m8_3_tcpfix_30mm_stallmonitor_${TS}.log"
TRAJ_CSV="runs/m8_3_tcpfix_30mm_traj_${TS}.csv"
TRAJ_LOG="${LOGDIR}/m8_3_tcpfix_30mm_trajrecorder_${TS}.log"
CONTACT_L="${LOGDIR}/m8_3_tcpfix_30mm_contact_left_${TS}.log"
CONTACT_R="${LOGDIR}/m8_3_tcpfix_30mm_contact_right_${TS}.log"
POSE_CSV="runs/m8_3_tcpfix_30mm_pose_${TS}.csv"
POSE_LOG="${LOGDIR}/m8_3_tcpfix_30mm_poserecorder_${TS}.log"
MARKER_DIR="${LOGDIR}/m8_3_tcpfix_30mm_markers_${TS}"
CSV_PATH="m3_grasp_trace_m8_3_tcpfix_30mm_${TS}.csv"
