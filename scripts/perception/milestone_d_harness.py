...
# ===========================================================================
"""md_harness.py -- Milestone D evaluation harness.  NOT part of the estimator.
...
class Harness(Node):
    def __init__(self):
        ...
        self.positions = []   # (stamp_sec, x, y, z, recv_walltime)
    ...
    def fresh_positions(self, t_min_sim, count, skip, timeout=60.0):
        """Estimates whose OWN observation stamp is after the arm stopped."""
        start = len(self.positions)
        end = time.time() + timeout
        got = []
        while time.time() < end and rclpy.ok() and len(got) < count + skip:
            rclpy.spin_once(self, timeout_sec=0.1)
            for rec in self.positions[start:]:
                if rec[0] > t_min_sim and rec not in got:
                    got.append(rec)
        return got[skip:skip + count], got
...
def main():
    ...
    if a.absent:
        ...
        fresh, allseen = h.fresh_positions(t_stop, a.frames, skip=1)
        log["fresh_count"] = len(fresh)
        log["estimates"] = [{"stamp": p[0], "x": p[1], "y": p[2], "z": p[3],
                             "frame_id": p[5]} for p in fresh]
        log["discarded_stale_or_warmup"] = len(allseen) - len(fresh)
        for i, p in enumerate(fresh):
            print(f"[observe] frame {i}: stamp={p[0]:.6f} frame_id={p[5]} "
                  f"XYZ=({p[1]:.9f}, {p[2]:.9f}, {p[3]:.9f})")
